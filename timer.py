...
class Timer:
    ...
    def __init__(self):
        print("Loading")
        d.init()
        schedule.every(5).seconds.do(self.update_rss)

        schedule.every(1).hour.do(self.update_weather)
        #schedule.every(1).seconds.do(loadRssData)

        schedule.every().day.at("00:00").do(self.update_date)
        ...
        schedule.every(1).seconds.do(self.update_time)
        schedule.every(1).seconds.do(self.update_display)
        schedule.every(5).minutes.do(self.update_mood)
